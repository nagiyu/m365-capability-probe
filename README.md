...
**1 回の実行に 2 レグ。** 同じ呼び出しを、ヘッダー無しで 1 回、有りで 1 回。トークンもドライブも
分も動かさないので、**レグの差はヘッダーだけ**です。**トークン無しの `delta`**（全件列挙）を使います
― トークン付きは「前回から何が変わったか」に答えるもので、訊きたいことが違うので。
...
> **黙って受理されたのと、黙って無視されたのは、ここからは同じに見えます。** 認識できない
