...
**共有リンクの裏のグループが空なのは、正常です。** SharePoint はこのグループをリンクの名前で
呼びます ― `SharingLinks.<アイテム>.**OrganizationEdit**.<リンク>` ― そして**名前が対象範囲を
持っています**。組織全体に向いたリンクには列挙する相手がいないので、**空を「誰も開けない」と
読むと逆になります**。対象範囲は名前から**引用するだけ**で解釈しません。それぞれの対象範囲が
実際に何を許すかは、まだ測っていないので。
