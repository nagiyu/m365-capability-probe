...
**対象グループは名前つきで記録します** (`group  id 13, "…" (discovered by …)`)。サイトのグループは
互換ではなく、**管理者が作って人を入れたもの**と **SharePoint が自分の都合で作ったもの**が混ざります。
`0 メンバー` は後者については当たり前の事実で、前者については驚くべき事実です ― 番号だけでは
読み手にその区別がつきません。実際に一度、**それを取り違えかけました**。
