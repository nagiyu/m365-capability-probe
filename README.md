...
| `scope` | 到達を増やすか |
| --- | --- |
| `anonymous` | **はい**（テナント外まで） |
| `organization` | **はい**（テナント内・列挙不可） |
| `users` | **はい**（名前が付く） |
| `existingAccess` | **いいえ。**元から開ける人にしか効きません |
| 上記以外 | **不明**。どちらとも書きません |

**`existingAccess` が本命です。** 何も与えないのに裏のグループはできるので、**リンクの裏の
グループを到達として数えると、このリンクに存在しない対象範囲を与えたことになります。**
